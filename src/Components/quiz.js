...
function Quiz() {
    //В React хуки, такие как useState, позволяют управлять состоянием в функциональных компонентах. Я добавил хук useState в компоненте Quiz, чтобы отслеживать:
// Текущий шаг в квизе (step), определяя, на каком вопросе находится пользователь.
// Количество правильных ответов (correct), чтобы подсчитать их и отобразить в результате.
